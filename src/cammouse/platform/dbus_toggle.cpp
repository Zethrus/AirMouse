...
class DbusToggleImpl final : public DbusToggle {
 public:
  ...
  bool advertise() override {
    ...
    dbus_bus_request_name(conn_, "org.cammouse.App", DBUS_NAME_FLAG_REPLACE_EXISTING, &err);
    if (dbus_error_is_set(&err)) dbus_error_free(&err);
    dbus_bus_add_match(conn_,
                       "type='method_call',interface='org.cammouse.App',path='/org/cammouse/App'",
                       nullptr);
    return true;
  }
  ...
  void poll() override {
    ...
    while ((msg = dbus_connection_pop_message(conn_)) != nullptr) {
      if (dbus_message_is_method_call(msg, "org.cammouse.App", "TogglePause")) {
